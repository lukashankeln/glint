...
> **Not yet implemented.** Schema validation is on the roadmap. The config fields are accepted and ignored — they will activate once the validator is wired in.
...
schema:
  ...
  ignore_missing_schemas: true     # Don't error on unknown CRD kinds
  skip_kinds:
    - CustomResourceDefinition     # Kinds to skip schema validation for
  reject_kinds: []                 # Treat these kinds as errors even without a schema
  locations: []                    # Prepend custom schema registry URLs
  cache_dir: "~/.glint/schema-cache"
```
