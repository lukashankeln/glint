...
func Discover(ctx context.Context, paths []string, cfg *config.Config) ([]DiscoveredApp, error) {
	...
	for _, root := range paths {
		...
		err = filepath.WalkDir(abs, func(path string, d fs.DirEntry, err error) error {
			...
			crdFound, err := processYAMLFile(path, repoRoot, helmRepos, &apps, seen)
			if err != nil {
				log.Warn().Err(err).Str("file", path).Msg("skipping file with parse errors")
				return nil
			}

			// If no CRD was found, the directory is a candidate for a raw YAML app.
			if !crdFound {
				rawDirs[dir] = true
			}
	...
	for {
		...
		case FrameworkArgoCD:
			if strings.ToLower(md.Kind) == "applicationset" {
				...
				// Skip creating a DiscoveredApp; the directory will be picked up
				// as raw YAML so the manifest is still available for schema validation.
				continue
			}
			crdFound = true
